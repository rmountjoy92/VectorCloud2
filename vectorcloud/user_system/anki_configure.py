...
def get_session_token(api, username=None, password=None):
    ...
    if not password:
        ...
        print("Using password from command line: {}".format(colored(password, "cyan")))
...
def main(api):
    ...
    parser.add_argument(
        "-p", "--password", help="The password used by your Anki account."
    )
    parser.add_argument(
        "-i",
        "--ip",
    ...
    token = get_session_token(api, args.email, args.password)
